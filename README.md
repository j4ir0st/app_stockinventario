...
| :--- | :--- | :--- |
| **0.8.2** | 2026-09-16 | **Alternancia Dinámica de Fechas en Detalle (Stock_ERP)**: Visualización por defecto de **Fecha de Vencimiento (`fecha_venc`)** en el resumen agrupado de almacenaje, alternando a **Fecha Movimiento (`fecha_movimiento`)** de forma contextual al hacer clic en el botón de registros (kardex original). El ordenamiento reactivo en memoria se adapta automáticamente a la fecha activa. |
| **0.8.1** | 2026-08-17 | **Mapeo de Proveedor Consolidado en Tránsito**: Se integró el mapeo automático del campo `prov` de `SI_Transito` utilizando la tabla `SI_Proveedor` guardada en caché (`SI_Transito.prov = SI_Proveedor.nombre → SI_Proveedor.consolidado`), asegurando que en la vista principal y reportes Excel de tránsito aparezca el nombre consolidado del proveedor. |
| **0.8.0** | 2026-08-11 | **Integración de Stock en Tránsito (/SI_Transito/)**: Integración automática de registros de tránsito en la vista principal de `StockInventario` y en la exportación a Excel. Se contrasta por `tipo` y `codigo`, insertando una fila consolidada por empresa (`[Nombre Empresa] \| STOCK EN TRANSITO`) antes de la suma del subtotal por código, recalculando el subtotal para que todas las cuentas calcen. Carga y caché de la tabla `/Empresa/` en `FilterDataService`. |
